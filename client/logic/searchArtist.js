...
async function searchArtist(query) {
  ...
  const res = await fetch(
    `http://localhost:3000/api/search/artist?q=${encodeURIComponent(query)}`,
  );
  const artists = await res.json();

  // artists est un tableau avec { name, images, followers, genres, ... }
  console.log(artists);
}

// Appel sur chaque frappe avec un délai de 400ms
inputSearch.addEventListener("input", (e) => {
  clearTimeout(debounceTimer);
  debounceTimer = setTimeout(() => searchArtist(e.target.value), 400);
});
